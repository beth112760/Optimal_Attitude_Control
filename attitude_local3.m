...
function [R,J] = attitude_local3(x,BC,D,N,Neq,ns,M,ip,J,J_local,Elb,Erb,SLb,SRb,R,LJ_local,BCtype)
global u1 u2 u3
...
for k = 1 : M % for each cover
   ...
   if k == 2
        ...
        R(4*N+1:5*N,k) = D(:,:,k)*w1(:,k);
        R(5*N+1:6*N,k) = D(:,:,k)*w2(:,k);
        R(6*N+1:7*N,k) = D(:,:,k)*w3(:,k);
        % L1 = Lq1, L2 = Lq2, L3 = Lq3, L4 = Lq4, L5 = Lw1, L6 = Lw2, L7 = Lw3
...
switch BCtype
    case "fixed" %fixed final state
        ...
        %R(5*N,1) = w1(N,1) - w_lim;
       % R(6*N,1) = w2(N,1) - w_lim;
        R(7*N,1) = w3(N,1) - w_lim;

       % R(4*N+1,2) = w1(1,2) - w_lim;
       % R(5*N+1,2) = w2(1,2) - w_lim;
    %    R(6*N+1,2) = w3(1,2) - w_lim;

       % R(5*N,2) = w1(N,2) - w_lim;
        %R(6*N,2) = w2(N,2) - w_lim;
    %   R(7*N,2) = w3(N,2) - w_lim;

       % R(4*N+1,end)   = w1(1,end) - w_lim;
       %R(5*N+1,end)   = w2(1,end) - w_lim;
       R(6*N+1,end)   = w3(1,end) - w_lim;
